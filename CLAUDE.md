...
- LLM responses are cached on disk (`cache.py` `ResponseCache`, default dir `$XDG_CACHE_HOME/doc_checker` or `~/.cache/doc_checker`) keyed by `sha256(model + "\0" + prompt)`; since the prompt embeds signature/docstring/code-excerpt the cache self-invalidates on any change. `_check_api_info` reads/writes it; error responses are never cached (so retries can succeed). `--no-cache` disables it (threads `quality_use_cache` → `LLMQualityChecker.use_cache`). Default-disabled in `QualityChecker` (cache=None) so unit tests don't touch disk.
