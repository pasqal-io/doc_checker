...
- `--quality-min-severity` (`suggestion`|`warning`|`critical`) filters quality issues in `LLMQualityChecker.check()` via `SEVERITY_RANK` before they reach the report, so it also affects exit code; unknown severities are always kept. CLI default is `critical` (quiet); the library default (`check_all`/`LLMQualityChecker`) stays `suggestion`=keep-all
