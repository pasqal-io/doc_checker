...
- LLM responses can be cached on disk (`cache.py` `ResponseCache`, default dir `$XDG_CACHE_HOME/doc_checker` or `~/.cache/doc_checker`) keyed by `sha256(model + "\0" + prompt)`; since the prompt embeds signature/docstring/code-excerpt the cache self-invalidates on any change. `_check_api_info` reads/writes it; error responses are never cached (so retries can succeed). **Caching is OFF by default everywhere** (CLI `--cache`/`--no-cache` via `BooleanOptionalAction` default False; `check_all` `quality_use_cache=False`; `LLMQualityChecker.use_cache=False`) because per-run recall is stochastic and the cache would freeze a run's misses. `quality_use_cache` threads → `LLMQualityChecker.use_cache`. `QualityChecker` takes `cache=None` (disabled) so unit tests don't touch disk.
...
Tests use `tmp_path` fixtures in `conftest.py` (`tmp_docs`, `sample_markdown`, `sample_notebook`, `sample_mkdocs_yml`). Each test creates isolated temp project structure with fake modules and docs. Test files mirror source modules: `test_checkers.py`, `test_parsers.py`, `test_code_analyzer.py`, `test_link_checker.py`, `test_external_links.py`, `test_llm_checker.py`, `test_llm_backends.py`, `test_prompts.py`, `test_cache.py`, `test_integration.py`.
