...
- Code-vs-docstring alignment: `CodeAnalyzer._get_source_excerpt()` captures up to `MAX_SOURCE_LINES` (25) of source via `inspect.getsource` into `SignatureInfo.source_excerpt` (classes prefer their own `__init__`); forwarded as the prompt's `code_snippet`. Inherited/parent-class behavior is still a blind spot.
